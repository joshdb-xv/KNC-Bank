...
export default function Deposit() {
    ...
    const handleDeposit = async () => {
        ...
        if (amount < 100) {
            showToast('Minimum deposit amount is PHP 100', 'error');
            return;
        }
        ...
        try {
            const response = await fetch('http://localhost:8000/auth/deposit', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                showToast(`Successfully deposited PHP ${amount.toFixed(2)}`, 'success', 5000);
                ...
                showToast(data.detail || 'Deposit failed', 'error');
            }
        } catch (error) {
        ...
        <div className="flex bg-gray">
            ...
            <div className="w-4/5 flex h-screen flex-col items-center">
                ...
                <div className="flex items-center w-full h-24 bg-white shadow-[0px_2px_16px_0px_rgba(0,0,0,0.15)]">
                    <h1 className='text-primary text-4xl font-semibold mx-8'>Deposit</h1>
                </div>
                ...
                <p className="text-lg text-black font-medium tracking-widest mt-8">Minimum of PHP 1000 to deposit.</p>
            </div>
        </div>
    );
}